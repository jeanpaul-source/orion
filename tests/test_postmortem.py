...
# ---------------------------------------------------------------------------
# 7. /postmortem with no description prints usage
# ---------------------------------------------------------------------------


def test_cmd_postmortem_no_description_prints_usage():
    import hal.main as main_mod

    printed: list[str] = []

    with patch.object(
        main_mod.console, "print", side_effect=lambda msg, **_: printed.append(str(msg))
    ):
        main_mod.cmd_postmortem(
            raw_args="",
            prom=MagicMock(),
            executor=MagicMock(),
            judge=MagicMock(),
    ...
    assert any("Usage" in line or "usage" in line for line in printed)


# ---------------------------------------------------------------------------
# 8. --hours parsing extracts window and strips it from description
# ---------------------------------------------------------------------------


def test_cmd_postmortem_hours_parsing():
    import hal.main as main_mod

    captured_calls: list[dict] = []

    def fake_gather(description, window_hours, prom, executor, judge):
        captured_calls.append(
            {"description": description, "window_hours": window_hours}
        )
        return "=== POST-INCIDENT EVIDENCE ===\nfake context"

    with (
        patch.object(main_mod, "gather_postmortem_context", fake_gather),
        patch("hal.agent.run_agent"),
        patch.object(main_mod.console, "print"),
    ):
        main_mod.cmd_postmortem(
            raw_args="disk full --hours 6",
            prom=MagicMock(),
            executor=MagicMock(),
            judge=MagicMock(),
            history=[],
    ...
    assert captured_calls[0]["window_hours"] == 6
    assert captured_calls[0]["description"] == "disk full"
